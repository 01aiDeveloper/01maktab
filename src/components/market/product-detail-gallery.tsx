...
const statusLabel: Record<ProductStatus, React.ReactNode> = {
  ...
  COIN_ONLY: (
    <span className="inline-flex items-center gap-1">
      Только за <Image quality={90} src="/images/market/VC.png" alt="VC" width={15} height={20} className="w-4 h-5 align-middle" />
    </span>
  ),
  SOON: 'Скоро',
};
      ...
      {product.images.length > 1 && (
        <div className="flex items-center gap-2 overflow-x-auto pb-1 shrink-0 scrollbar-hide">
          {product.images.map((img, i) => (
            ...
            >
              <Image quality={90} src={img} alt="" fill sizes="33vw" className="object-cover object-top" />
            </button>
          ))}
        </div>
      )}
