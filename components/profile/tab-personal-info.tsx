...
export function TabPersonalInfo() {
  ...
  const inputCls = "w-full h-11 px-4 rounded-xl border border-gray-200 bg-white text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-[#3B5BFF]/20 focus:border-[#3B5BFF]"
            ...
            <button className="relative" onClick={handleAvatarClick}>
              <div className="w-20 h-20 rounded-full bg-gray-100 overflow-hidden flex items-center justify-center">
                {avatarPreview || user?.avatar ? (
                  <Image src={avatarPreview || getMediaUrl(user!.avatar) || ''} alt="avatar" width={80} height={80} className="w-full h-full object-cover" />
                ) : (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-5">
            ...
            <div>
              ...
              <div className="flex rounded-xl border border-gray-200 overflow-hidden h-11">
                <button type="button" onClick={() => setForm({ ...form, gender: 'MALE' })} className={`flex-1 text-sm font-medium transition-colors ${form.gender === 'MALE' ? 'bg-gray-900 text-white' : 'bg-white text-gray-600'}`}>Erkak</button>
                <button type="button" onClick={() => setForm({ ...form, gender: 'FEMALE' })} className={`flex-1 text-sm font-medium transition-colors ${form.gender === 'FEMALE' ? 'bg-gray-900 text-white' : 'bg-white text-gray-600'}`}>Ayol</button>
              </div>
            </div>
            ...
            <div>
              ...
              <input type="date" value={form.birthday ? form.birthday.split('T')[0] : ''} onChange={(e) => setForm({ ...form, birthday: e.target.value })} className={inputCls} />
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-[22px] p-6 lg:p-10 shadow-[0_1px_4px_rgba(0,0,0,0.04)]">
        ...
        <div className="flex justify-center lg:justify-start mb-8">
          <button className="relative" onClick={handleAvatarClick}>
            ...
            <span className="absolute -bottom-1 -right-1 w-7 h-7 bg-[#3B5BFF] rounded-full flex items-center justify-center">
              <Pencil className="w-3.5 h-3.5 text-white" />
            </span>
          </button>
        </div>
